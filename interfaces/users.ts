...
export default class Users {
  ...
  static async checkValidApiKey(req: Request) {
    ...
    try {
      const user = (
        await query(
          'SELECT `users`.`id` AS `id`, `username`, `permission_code` FROM `users`, `api_keys` WHERE `api_key` = ? AND `users`.`id` = `api_keys`.`user_id` AND `date_created` > CURRENT_TIMESTAMP - INTERVAL ? DAY',
          [md5(apiKey), API_EXPIRY_DAYS],
        )
      )[0]
  ...
  static async logoutAll(req: Request, res: Response) {
    ...
    try {
      await query('DELETE FROM `api_keys` WHERE `user_id` = (SELECT `user_id` FROM `api_keys` WHERE `api_key` = ?)', [
        md5(apiKey),
      ])
      return res.status(200).json({})
    } catch (_) {
