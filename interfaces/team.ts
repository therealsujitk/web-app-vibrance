...
export default class Team {
  ...
  async #get(id: number) {
    const member = (
      await query(
        'SELECT `name`, `team_name`, `role`, `image`, `phone`, `email` FROM `team` LEFT JOIN `images` ON `image_id` = `images`.`id` WHERE `team`.`id` = ?',
        [id],
      )
    )[0]
  ...
  async add(team: TeamModel) {
    ...
    const queries = [
      ...
      {
        query:
          'INSERT INTO `team` (`name`, `team_name`, `role`, `image_id`, `phone`, `email`) VALUES (?, ?, ?, ?, ?, ?)',
        options: (results: any[]) => [
          team.name,
          team.team_name,
          team.role,
  ...
  async edit(id: number, team: OrNull<TeamModel>) {
    ...
    const queries = [
      ...
      {
        query:
          'UPDATE `team` SET `name` = ?, `team_name` = ?, `role` = ?, `image_id` = ?, `phone` = ?, `email` = ? WHERE `id` = ?',
        options: (results: any[]) => [
          team.name,
          team.team_name,
          team.role,
