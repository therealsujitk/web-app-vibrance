          ...
          <form ref={this.formRef} onSubmit={(event) => event.preventDefault()}>
            ...
            <Stack spacing={1} mt={0.5}>
              ...
              <Autocomplete
                ...
                renderInput={(params) => (
                  <>
                    ...
                    {selectedPermissions.map(p => <input name="permissions[]" value={p} hidden />)}
                  </>
                )}
              ...
              {user && <FormControlLabel
                control={
                  <Checkbox defaultChecked={isPasswordEditable} />
                }
              ...
              <Button 
                type="submit" 
                isLoading={isLoading} 
                variant="contained" sx={(theme) => ({ mt: `${theme.spacing(2)} !important` })}
                onClick={() => {
                  setLoading(true);
      ...
      <Dialog onClose={onClose} open={this.state.isDeleteDialogOpen}>
        ...
        <DialogContent>
          ...
          <Stack spacing={2} mt={0.5}>
            <Typography>Are you sure you want to delete <b>{user?.username}</b>?</Typography>
            <Button 
              isLoading={isLoading}
              variant="contained"
              onClick={() => {
                setLoading(true);
                this.deleteItem(user!.id).then(_ => onClose()).finally(() => setLoading(false));
              }}
            >Delete User</Button>
          </Stack>
        </DialogContent>
  ...
  render() {
      ...
      },
      {
        ...
        renderCell: (params) => {
          return (<Box data-field="actions-wrapper">
            <MaterialButton variant="contained" color="warning" sx={{ mr: 1 }} startIcon={<Edit />} onClick={() => this.setState({editingUser: params.row.user, isAddOrEditDialogOpen: true})}>Edit</MaterialButton>
            <MaterialButton variant="contained" color="error" startIcon={<Delete />} onClick={() => this.setState({editingUser: params.row.user, isDeleteDialogOpen: true})}>Delete</MaterialButton>
          </Box>);
        }
      }
    ];
    ...
    return (
      <Box>
        ...
        <PanelHeader {...Drawer.items.users} action={<MaterialButton variant="outlined" startIcon={<AddIcon />} onClick={() => this.setState({editingUser: undefined, isAddOrEditDialogOpen: true})}>Add User</MaterialButton>} />
        <Box sx={{p: 2}}>
