...
import { Avatar, Box, Button as MaterialButton, Card, CardActions, CardContent, Chip, CircularProgress, DialogContent, IconButton, Stack, Tooltip, Typography, Autocomplete } from "@mui/material";
...
export default class TeamPanel extends BasePanel<{}, TeamPanelState> {
    ...
    return (
      <Card>
        <CardContent sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <Typography variant="body1" textAlign="center">{member.role}</Typography>
          <Chip 
  ...
  AddOrEditDialog = () => {
    ...
    const teamNames = !selectedTeamName || this.state.teamNames.has(selectedTeamName) ? Array.from(this.state.teamNames) : [selectedTeamName, ...Array.from(this.state.teamNames)];
            ...
            <AppContext.Consumer>
              {({ displayError }) => (
                <Stack spacing={1} mt={0.5}>
                  <ImageInput name="image" defaultValue={member?.image ?? undefined} onError={displayError} disabled={isLoading} />
                  ...
                  <Autocomplete
                    ...
                    renderInput={(params) => 
                      <TextField {...params} placeholder="Team Name" name="team_name" onChange={(e) => setTeamName(e.target.value)} />
                    }
                    disabled={isLoading}
                  />
                  <TextField name="role" placeholder="Role" defaultValue={member?.role} disabled={isLoading} />
                  <TextField name="phone" placeholder="Mobile Number" defaultValue={member?.phone} disabled={isLoading} />
                  <TextField name="email" placeholder="Email ID" type="email" defaultValue={member?.email} disabled={isLoading} />
                  <Button 
                    type="submit" 
                    isLoading={isLoading} 
                    variant="contained" 
      ...
      <Dialog onClose={onClose} open={this.state.isDeleteDialogOpen}>
        ...
        <DialogContent>
          ...
          <Stack spacing={2} mt={0.5}>
            <Typography>Are you sure you want to delete <b>{member?.name}</b>?</Typography>
            <Button 
              isLoading={isLoading} 
              variant="contained" 
              onClick={() => {
                setLoading(true);
                this.deleteItem(member!.id).then(_ => onClose()).finally(() => setLoading(false));
              }}
  ...
  render() {
    return (
      <Box>
        ...
        <PanelHeader {...Drawer.items.team} action={<MaterialButton variant="outlined" startIcon={<AddIcon />} onClick={() => this.setState({editingMember: undefined, isAddOrEditDialogOpen: true})}>Add Member</MaterialButton>} />
