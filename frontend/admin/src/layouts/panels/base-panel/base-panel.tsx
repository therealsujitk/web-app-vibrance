...
export abstract class BasePanel<P = {}, S extends BasePanelState = BasePanelState, SS = any> extends React.Component<
  P,
  S,
  SS
> {
  ...
  addOrEditItem = async (patchEndpoint = this.patchEndpoint(), putEndpoint = this.putEndpoint()) => {
    try {
      ...
      if (formData.get('id')) {
        const response = await new Network(this.apiKey).doPatch(
          patchEndpoint,
          { body: formData },
          this.requireMultipart,
        )
