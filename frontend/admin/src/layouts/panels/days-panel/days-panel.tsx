          ...
          <form ref={this.formRef} onSubmit={(event) => event.preventDefault()}>
            ...
            <Stack spacing={1} mt={0.5}>
              ...
              <TextField
                ...
                type="date"
                name="date"
                defaultValue={day?.date ? format(day.date, 'yyyy-MM-dd') : ''}
                disabled={isLoading}
              />
              <Button
                type="submit"
      ...
      <Dialog onClose={onClose} open={this.state.isDeleteDialogOpen}>
        ...
        <DialogContent>
          ...
          <Stack spacing={2} mt={0.5}>
            <Typography>
              Are you sure you want to delete <b>{day?.title}</b>?
            </Typography>
            <Button
              isLoading={isLoading}
              variant="contained"
        ...
        <PanelHeader
          ...
          action={
            <MaterialButton
              variant="outlined"
              startIcon={<AddIcon />}
              onClick={() => this.setState({ editingDay: undefined, isAddOrEditDialogOpen: true })}
            >
              Add Day
            </MaterialButton>
          }
        ...
        <Box sx={{ pl: 2, pt: 2, overflowAnchor: 'none' }}>
          {this.state.isLoading || this.state.days.size != 0 ? (
            <Masonry columns={{ xs: 1, sm: 2, md: 2, lg: 2, xl: 4 }} spacing={2}>
              {Array.from(this.state.days).map(([_, day]) => (
                <this.DayCard {...day} />
              ))}
            </Masonry>
          ) : (
