...
export default class SponsorPanel extends BasePanel<{}, SponsorsPanelState> {
  ...
  SponsorCard = (sponsor: Sponsor) => {
    return (
      <Card>
        {sponsor.image && <CardMedia component="img" height="120" image={sponsor.image} />}
        <CardContent>
            ...
            <AppContext.Consumer>
              {({ displayError }) => (
                <Stack spacing={1} mt={0.5}>
                  <ImageInput
                    name="image"
                    defaultValue={sponsor?.image ?? undefined}
                    onError={displayError}
                    disabled={isLoading}
                  />
                  <TextField name="title" placeholder="Title" defaultValue={sponsor?.title} disabled={isLoading} />
                  <Select name="type" defaultValue={sponsor?.type.toLowerCase() ?? 0} disabled={isLoading}>
                    <MenuItem value="0" disabled>
                      Select Type
                    </MenuItem>
                    {this.state.sponsorTypes!.map((type) => (
                      <MenuItem value={type.toLowerCase()}>{getSponsorType(type)}</MenuItem>
                    ))}
                  </Select>
                  <TextArea
                    name="description"
      ...
      <Dialog onClose={onClose} open={this.state.isDeleteDialogOpen}>
        ...
        <DialogContent>
          ...
          <Stack spacing={2} mt={0.5}>
            <Typography>
              Are you sure you want to delete <b>{sponsor?.title}</b>?
            </Typography>
            <Button
              isLoading={isLoading}
              variant="contained"
        ...
        <PanelHeader
          ...
          action={
            <MaterialButton
              variant="outlined"
              startIcon={<AddIcon />}
              onClick={() => this.setState({ editingSponsor: undefined, isAddOrEditDialogOpen: true })}
            >
              Add Sponsor
            </MaterialButton>
          }
