import AuditLogPanel from "./audit-log-panel/audit-log-panel";
import CategoriesPanel from "./categories-panel/categories-panel";
import DashboardPanel from "./dashboard-panel/dashboard-panel";
import DaysPanel from "./days-panel/days-panel";
import EventsPanel from "./events-panel/events-panel";
import GalleryPanel from "./gallery-panel/gallery-panel";
import MerchandisePanel from "./merchandise-panel/merchandise-panel";
import ProShowsPanel from "./pro-shows-panel/pro-shows-panel";
import SettingsPanel from "./settings-panel/settings-panel";
import SponsorsPanel from "./sponsors-panel/sponsors-panel";
import TeamPanel from "./team-panel/team-panel";
import UsersPanel from "./users-panel/users-panel";
import VenuesPanel from "./venues-panel/venues-panel";

export { AuditLogPanel, CategoriesPanel, DaysPanel, EventsPanel, DashboardPanel, GalleryPanel, MerchandisePanel, ProShowsPanel, SettingsPanel, SponsorsPanel, TeamPanel, UsersPanel, VenuesPanel };
