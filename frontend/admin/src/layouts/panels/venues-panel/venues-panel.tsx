      ...
      <Card>
        <CardContent>
          ...
          <Stack direction="row" sx={{ pt: 1, flexWrap: 'wrap', gap: 1 }}>
            <Chip icon={<AddIcon />} label="Add room" variant="outlined" onClick={() => this.setState({editingVenue: venue, editingRoom: undefined, isEditingRoom: true, isAddOrEditDialogOpen: true})} />
            {Object.values(venue.rooms).flatMap((r, i) => 
              r.title != null ? <Chip key={i} label={r.title} onClick={() => this.setState({editingVenue: venue, editingRoom: r, isEditingRoom: true, isAddOrEditDialogOpen: true})} onDelete={() => this.setState({editingVenue: venue, editingRoom: r, isEditingRoom: true, isDeleteDialogOpen: true})} /> : []
            )}
          </Stack>
        </CardContent>
    ...
    return (
      <Dialog onClose={onClose} open={this.state.isAddOrEditDialogOpen}>
        <DialogTitle onClose={onClose}>
          {room?.id || (venue?.id && !this.state.isEditingRoom) ? 'Edit' : 'Add'} {this.state.isEditingRoom ? 'Room' : 'Venue'}
        </DialogTitle>
        <DialogContent>
          <form ref={this.formRef} onSubmit={(event) => event.preventDefault()}>
            ...
            <input name="id" value={this.state.isEditingRoom ? room?.id : venue?.id} type="hidden" />
              <Stack spacing={1} mt={0.5}>
                ...
                <TextField placeholder="Title" name="title" defaultValue={this.state.isEditingRoom ? room?.title : venue?.title} disabled={isLoading} />
                <Button 
                  type="submit" 
                  isLoading={isLoading} 
                  variant="contained" 
                  ...
                  onClick={() => {
                    ...
                    this.addOrEditItem(patchEndpoint, putEndpoint).then(_ => onClose()).finally(() => setLoading(false));
                  }}
                >Save {this.state.isEditingRoom ? 'Room' : 'Venue'}</Button>
              </Stack>
          </form>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent>
          <form ref={this.formRef} onSubmit={(event) => event.preventDefault()}>
            ...
            <Stack spacing={2} mt={0.5}>
              <Typography>Are you sure you want to delete {this.state.isEditingRoom && <><b>{room?.title}</b> under </>}<b>{venue?.title}</b>?</Typography>
              <Button 
                type="submit" 
                isLoading={isLoading} 
                variant="contained" 
              ...
              >Delete {this.state.isEditingRoom ? 'Room' : 'Venue'}</Button>
            </Stack>
          </form>
        </DialogContent>
      </Dialog>
  ...
  render() {
    return (
      <Box>
        ...
        <PanelHeader {...Drawer.items.venues} action={<MaterialButton variant="outlined" startIcon={<AddIcon />} onClick={() => this.setState({editingVenue: undefined, isEditingRoom: false, isAddOrEditDialogOpen: true})}>Add Venue</MaterialButton>} />
