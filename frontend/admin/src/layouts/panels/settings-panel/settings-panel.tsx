...
export default class SettingsPanel extends React.Component<{}, SettingsPanelState> {
  ...
  Label = (props: { text: string; mt?: number }) => {
    return (
      <Typography variant={'body1'} sx={{ fontWeight: 'bold', mt: props.mt ?? 2 }}>
        {props.text}
      </Typography>
    )
  }
  ...
  Input = (props: { settingsKey: string }) => {
    return (
      <TextField
        size="small"
        sx={{ mt: 1, width: '100%' }}
        value={this.state.settings[props.settingsKey] ?? ''}
        onChange={(event) => this.handleChange(event, props.settingsKey)}
      />
    )
  }
  ...
  saveSettings = async () => {
    ...
    try {
      const response = await new Network(this.apiKey).doPatch(`${this.apiEndpoint}/edit`, {
      ...
      <Box>
        ...
        {this.state.isLoading ? (
          ...
          >
            ...
            <TextArea
              value={this.state.settings.site_description ?? ''}
              sx={{ mt: 1, p: 1, height: '75px' }}
              onChange={(event) => this.handleChange(event, 'site_description')}
            />
            <this.Label text="Read Only Mode" />
            <FormControlLabel
              control={
                <Switch
                  defaultChecked={this.state.settings.read_only}
                  onChange={(event) => (this.state.settings.read_only = event.target.checked)}
                />
              }
              label={<this.Subtitle text="If true, users will not be allowed to make any changes to the data." />}
            />
            <br />
            <Button
              variant="contained"
