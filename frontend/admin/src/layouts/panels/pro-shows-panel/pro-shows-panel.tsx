    ...
    return (
      <Card sx={{ display: 'flex', flexDirection: orientation }}>
        {proShow.image && <img 
          ...
          onLoad={onImageLoad} 
        />}
        {proShow.image && isLoaded && <CardMedia 
          sx={{ 
            minWidth: '40%',
          ...
          <CardContent>
            ...
            <Stack direction="row" sx={{ pt: 1, flexWrap: 'wrap', gap: 1, '& svg': {width: '18px'} }}>
              ...
              <Chip 
                ...
                sx={{ pl: 0.5 }}
              />
              {proShow.facultyCoordinatorName && <Chip 
                label={proShow.facultyCoordinatorName + (proShow.facultyCoordinatorMobile ? ` - ${proShow.facultyCoordinatorMobile}` : '')} 
                icon={<Face />} 
                sx={{ pl: 0.5 }}
              />}
              {proShow.studentCoordinatorName && <Chip 
                label={proShow.studentCoordinatorName + (proShow.studentCoordinatorMobile ? ` - ${proShow.studentCoordinatorMobile}` : '')} 
                icon={<Face />} 
                sx={{ pl: 0.5 }}
              />}
            </Stack>
          ...
          <form ref={this.formRef} onSubmit={(event) => event.preventDefault()}>
            ...
            <Stack direction="row" spacing={1} sx={{ alignItems: 'stretch' }}>
              <Stack spacing={1} sx={{ flexGrow: 1, width: '40%' }}>
                <ImageInput name="image" style={{ height: 'unset', flexGrow: 1 }} defaultValue={proShow?.image ?? undefined} disabled={isLoading} />
                <TextField name="title" placeholder="Title" defaultValue={proShow?.title} disabled={isLoading} />
                <TextArea name="description" placeholder="Add a description..." style={{ minWidth: '100%' }} defaultValue={proShow?.description ?? undefined} disabled={isLoading} />
              </Stack>
              <Stack spacing={1} sx={{ flexGrow: 1, maxWidth: '60%' }}>
                ...
                <Autocomplete
                  ...
                  filterOptions={x => x}
                  defaultValue={proShow ? { id: proShow.roomId, title: proShow.venue + (proShow.room ? ' - ' + proShow.room : '') } : undefined}
                ...
                <Stack direction="row" spacing={1}>
                  <TextField name="start_time" placeholder="Start Time" type="time" defaultValue={proShow ? format(proShow.startTime, 'HH:mm') : ''} sx={{ flexGrow: 1 }} disabled={isLoading} />
                  <TextField name="end_time" placeholder="End Time" type="time" defaultValue={proShow ? format(proShow.endTime, 'HH:mm') : ''} sx={{ flexGrow: 1 }} disabled={isLoading} />
                </Stack>
                <TextField 
                  name="cost" 
                  placeholder="Cost" 
                  ...
                  disabled={isLoading}
                />
                <TextField name="faculty_coordinator_name" placeholder="Faculty Coordinator Name" defaultValue={proShow?.facultyCoordinatorName} disabled={isLoading} />
                <TextField name="faculty_coordinator_mobile" type="number" placeholder="Faculty Coordinator Mobile" defaultValue={proShow?.facultyCoordinatorMobile} disabled={isLoading} />
                <TextField name="student_coordinator_name" placeholder="Student Coordinator Name" defaultValue={proShow?.studentCoordinatorName} disabled={isLoading} />
                <TextField name="student_coordinator_mobile" type="number" placeholder="Student Coordinator Mobile" defaultValue={proShow?.studentCoordinatorMobile} disabled={isLoading} />
                <TextField name="event_id" type="number" placeholder="Event ID" defaultValue={proShow?.eventId} disabled={isLoading} />
              </Stack>
            </Stack>
      ...
      <Dialog onClose={onClose} open={this.state.isDeleteDialogOpen}>
        ...
        <DialogContent>
          ...
          <Stack spacing={2} mt={0.5}>
            <Typography>Are you sure you want to delete <b>{proShow?.title}</b>?</Typography>
            <Button 
              isLoading={isLoading} 
              variant="contained" 
              onClick={() => {
                setLoading(true);
                this.deleteItem(proShow!.id).then(_ => onClose()).finally(() => setLoading(false));
              }}
            >Delete Pro Show</Button>
          </Stack>
        </DialogContent>
      </Dialog>
    );
  ...
  render() {
    return (
      <Box>
        ...
        <PanelHeader {...Drawer.items['pro-shows']} action={<MaterialButton variant="outlined" startIcon={<AddIcon />} onClick={() => this.setState({editingProShow: undefined, isAddOrEditDialogOpen: true})}>Add Pro Show</MaterialButton>} />
