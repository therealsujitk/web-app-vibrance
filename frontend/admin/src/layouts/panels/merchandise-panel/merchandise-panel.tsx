...
export default class MerchandisePanel extends BasePanel<{}, MerchandisePanelState> {
  ...
  MerchandiseCard = (merchandise: Merchandise) => {
    return (
      <Card>
        {merchandise.image && <CardMedia component="img" height="120" image={merchandise.image} />}
        <CardContent>
            ...
            <AppContext.Consumer>
              {({ displayError }) => (
                <Stack spacing={1} mt={0.5}>
                  <ImageInput
                    name="image"
                    defaultValue={merchandise?.image ?? undefined}
      ...
      <Dialog onClose={onClose} open={this.state.isDeleteDialogOpen}>
        ...
        <DialogContent>
          ...
          <Stack spacing={2} mt={0.5}>
            <Typography>
              Are you sure you want to delete <b>{merchandise?.title}</b>?
            </Typography>
            <Button
              isLoading={isLoading}
              variant="contained"
              onClick={() => {
                setLoading(true)
                this.deleteItem(merchandise!.id)
                  .then((_) => onClose())
                  .finally(() => setLoading(false))
              }}
            >
  ...
  render() {
    return (
      <Box>
        ...
        <PanelHeader
          {...Drawer.items.merchandise}
          action={
            <MaterialButton
              variant="outlined"
              startIcon={<AddIcon />}
              onClick={() => this.setState({ editingMerchandise: undefined, isAddOrEditDialogOpen: true })}
            >
              Add Merchandise
            </MaterialButton>
          }
