        ...
        <Box sx={{ display: 'flex', flexDirection: 'column' }}>
          <CardContent>
            ...
            <Stack direction="row" sx={{ pt: 1, flexWrap: 'wrap', gap: 1, '& svg': { width: '18px' } }}>
              <Chip label={event.category} icon={Drawer.items.categories.icon} sx={{ pl: 0.5 }} />
              <Chip label={event.day} icon={Drawer.items.days.icon} sx={{ pl: 0.5 }} />
              <Chip
              ...
              {event.facultyCoordinatorName && (
                <Chip
                  label={
                    event.facultyCoordinatorName +
                    (event.facultyCoordinatorMobile ? ` - ${event.facultyCoordinatorMobile}` : '')
                  }
                  icon={<Face />}
                  sx={{ pl: 0.5 }}
                />
              ...
              {event.studentCoordinatorName && (
                <Chip
                  label={
                    event.studentCoordinatorName +
                    (event.studentCoordinatorMobile ? ` - ${event.studentCoordinatorMobile}` : '')
                  }
                  icon={<Face />}
                  sx={{ pl: 0.5 }}
                />
            ...
            <Stack direction="row" spacing={1} sx={{ alignItems: 'stretch' }}>
              ...
              <Stack spacing={1} sx={{ flexGrow: 1, maxWidth: '50%' }}>
                ...
                <Autocomplete
                  ...
                  defaultValue={
                    event
                      ? { id: event.roomId, title: event.venue + (event.room ? ' - ' + event.room : '') }
                      : undefined
                  }
                ...
                <Stack direction="row" spacing={1}>
                  <TextField
                    ...
                    type="number"
                    defaultValue={event?.teamSizeMin}
                    sx={{ flexGrow: 1 }}
                  />
                  <TextField
                    name="team_size_max"
                    ...
                    type="number"
                    defaultValue={event?.teamSizeMax}
                    sx={{ flexGrow: 1 }}
                  />
                </Stack>
                <Stack direction="row" spacing={1}>
                  <TextField
                    ...
                    type="time"
                    defaultValue={event ? format(event.startTime, 'HH:mm') : ''}
                    sx={{ flexGrow: 1 }}
                  />
                  <TextField
                    name="end_time"
                    ...
                    type="time"
                    defaultValue={event ? format(event.endTime, 'HH:mm') : ''}
                    sx={{ flexGrow: 1 }}
                  />
                </Stack>
                <TextField
        ...
        <PanelHeader
          ...
          action={
            <MaterialButton
              variant="outlined"
              startIcon={<AddIcon />}
              onClick={() => this.setState({ editingtEvent: undefined, isAddOrEditDialogOpen: true })}
            >
              Add Event
            </MaterialButton>
          }
          ...
          {this.state.isLoading || this.state.events.size != 0 ? (
            <Masonry columns={{ xs: 1, sm: 2, md: 2, lg: 2, xl: 4 }} spacing={2}>
              {Array.from(this.state.events).map(([_, event]) => (
                <div>
                  <this.EventCard key={event.id} {...event} />
                </div>
              ))}
            </Masonry>
          ) : (
