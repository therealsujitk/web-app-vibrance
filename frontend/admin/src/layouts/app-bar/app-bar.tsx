      ...
      <Dialog onClose={this.closeSignOutDialog} open={this.state.isSignOutDialogOpen || false}>
        ...
        <DialogContent>
          <Stack spacing={1} mt={0.5}>
            ...
            <AppContext.Consumer>
              {({ destroySession }) => (
                <Button variant="contained" onClick={() => this.signOut(destroySession)}>
                  Yes, I'm sure
                </Button>
              )}
            </AppContext.Consumer>
      ...
      <>
        ...
        <AppContext.Consumer>
          {({ username, setSession }) => (
            <EditProfileDialog
              username={username}
              opened={this.state.isEditProfileDialogOpen}
      ...
      <>
        ...
        >
          {Object.entries(this.props.items).map(([key, item]) => (
            ...
            >
              <span style={{ width: 24, marginRight: 12 }}>{item.icon}</span>
              {item.title}
            </MenuItem>
          ))}
        </Menu>
        ...
        <DialogContent>
          <form ref={this.formRef} onSubmit={(event) => event.preventDefault()}>
            <Stack spacing={1} mt={0.5}>
              ...
              <FormControlLabel
                control={<Checkbox defaultChecked={this.state.isPasswordEditable} />}
              ...
              <AppContext.Consumer>
                {({ displayError }) => (
                  <Button
                    ...
                    onClick={() => this.addEdit(displayError)}
                  >
                    Save Profile
                  </Button>
                )}
